...
func TestInitEmptyLister(t *testing.T) {
	...
	var err error
	_, err = newNRTCache(nil, fakeIndex)
	if err == nil {
		t.Fatalf("accepted nil lister")
	}

	_, err = newNRTCache(fakeInformer.Lister(), nil)
	if err == nil {
		t.Fatalf("accepted nil indexer")
	}
}

func TestDirtyNodeNamesCount(t *testing.T) {
	fakeClient := faketopologyv1alpha1.NewSimpleClientset()
	fakeInformer := topologyinformers.NewSharedInformerFactory(fakeClient, 0).Topology().V1alpha1().NodeResourceTopologies()
	fakeIndex := &fakePodByNodeNameIndex{}

	nrtCache, err := newNRTCache(fakeInformer.Lister(), fakeIndex)
	if err != nil {
		t.Fatalf("unexpected error creating cache: %v", err)
	}

	dirtyNodes := nrtCache.DirtyNodeNames()
	if len(dirtyNodes) != 0 {
...
func TestDirtyNodesMarkDiscarded(t *testing.T) {
	...
	for _, nodeName := range expectedNodes {
		nrtCache.MarkNodeDiscarded(nodeName)
	}
...
func TestGetByNode(t *testing.T) {
	...
	nrtObj = nrtCache.GetByNode("node1")
	if nrtObj != nil {
...
func TestGetByNodeReserve(t *testing.T) {
	fakeClient := faketopologyv1alpha1.NewSimpleClientset()
	fakeInformer := topologyinformers.NewSharedInformerFactory(fakeClient, 0).Topology().V1alpha1().NodeResourceTopologies()
	fakeIndex := &fakePodByNodeNameIndex{}
...
func TestGetByNodeReleaseNone(t *testing.T) {
	fakeClient := faketopologyv1alpha1.NewSimpleClientset()
	fakeInformer := topologyinformers.NewSharedInformerFactory(fakeClient, 0).Topology().V1alpha1().NodeResourceTopologies()
	fakeIndex := &fakePodByNodeNameIndex{}
	...
	nrtCache.ReleaseNodeResources("node1", testPod)
	...
	for _, zone := range nrtObj.Zones {
		for _, zoneRes := range zone.Resources {
			switch zoneRes.Name {
			case string(corev1.ResourceCPU):
				if zoneRes.Available.Cmp(resource.MustParse("30")) != 0 {
					t.Errorf("quantity mismatch in zone %q", zoneRes.Name)
				}
			case string(corev1.ResourceMemory):
				if zoneRes.Available.Cmp(resource.MustParse("60Gi")) != 0 {
					t.Errorf("quantity mismatch in zone %q", zoneRes.Name)
				}
			}
		}
...
func TestGetByNodeReserveRelease(t *testing.T) {
	fakeClient := faketopologyv1alpha1.NewSimpleClientset()
	fakeInformer := topologyinformers.NewSharedInformerFactory(fakeClient, 0).Topology().V1alpha1().NodeResourceTopologies()
	fakeIndex := &fakePodByNodeNameIndex{}
...
func TestFlush(t *testing.T) {
	...
	nrtCache.ReserveNodeResources("node1", testPod)
	nrtCache.MarkNodeDiscarded("node1")
	...
	nrtCache.FlushNodes(expectedNodeTopology.DeepCopy())
...
func (fi *fakePodByNodeNameIndex) ByIndex(indexName, indexedValue string) ([]interface{}, error) {
	if fi.err != nil {
		return nil, fi.err
	}
